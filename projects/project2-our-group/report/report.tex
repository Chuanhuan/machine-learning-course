...
\Authors{Bruno Magalhaes\textsuperscript{1}, Riccardo Silini{2} \hspace{7cm} team: deep mess} % Authors
...
Road classification from aerial pictures allow for automatic reconstruction of maps, better guidance of autonomous vehicles, among others. Manual classification is infeasible due to the large amount ot that. Automatic classification can be pursued with Machine Learning techniques.
...
Multi-core processing by setting \texttt{NUM\_THREADS} to the respective value of maximum concurrent threads in the processor's architecture. SEED, BATCH\_SIZE %"/model_phase_"+str(phase)+".ckpt"
